...
# Frozen first Phase-4a strict pair. Run from the repository root.
...
case "$ALPHA" in
  ...
  *)
    echo "Frozen Phase-4a screen permits ALPHA=0.25 or the predeclared 0.10 diagnostic" >&2
    exit 1
    ;;
esac

if [[ "$MAX_GUIDANCE_RATIO" != "0.25" ]]; then
  echo "Frozen Phase-4a screen requires MAX_GUIDANCE_RATIO=0.25" >&2
  exit 1
fi

PROJECT="project/geodata-3d-conditional"
RUNNER="$PROJECT/scripts/stage4/run_gravity_guidance.py"
AUDITOR="$PROJECT/scripts/stage4/audit_gravity_screen.py"
CASE="$PROJECT/samples/jupyter-demo/cond_generation_0"
CKPT="$PROJECT/demo_model/conditional-weights.ckpt"
OBSERVATION="$PROJECT/experiments/stage4_gravity/observations/cond_generation_0/distinct_upper_bound_v1_fix2"
CONTROLLER_MANIFEST="$PROJECT/experiments/stage4_gravity/configs/gravity_controller_manifest_v1.json"
RUNS_ROOT="$PROJECT/experiments/stage4_gravity/runs/cond_generation_0/phase4a_gravity_v1"
...
echo "[1/2] Phase-4a alpha-zero baseline: $BASELINE"
PYTHONPATH=src "$PYTHON_BIN" "$RUNNER" \
  "${COMMON[@]}" \
  --alpha 0 \
  --output-dir "$BASELINE"

echo "[2/2] Phase-4a $ALPHA_TAG: $GUIDED"
PYTHONPATH=src "$PYTHON_BIN" "$RUNNER" \
  "${COMMON[@]}" \
  --alpha "$ALPHA" \
...
echo "Completed Phase-4a strict pair. Audit with:"
