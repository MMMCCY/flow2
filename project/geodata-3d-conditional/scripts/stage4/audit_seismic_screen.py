#!/usr/bin/env python3
"""Audit a completed strict Phase-4c seismic pair."""
...
from guidance.seismic import SEISMIC_FORWARD_MODE, tensor_sha256
from scripts.stage2.summarize_phase2a import sample_gate_audit
from scripts.stage4.audit_gravity_screen import (
    _index_rows,
    _last_trace_rows,
    _mean,
    _phase2_baseline_regression,
    read_numeric_csv,
)
...
def _classification(
    ...
    if n_samples == 1:
        if pass_count == 1:
            return "single_sample_pass", "PASS: single-sample seismic screen; not confirmed"
        return "single_sample_failure", "FAIL: single-sample seismic screen"
    if n_samples == 4:
        if pass_count == 4:
            return "confirmed_seed42_pass", "PASS: confirmed seed-42 4/4 seismic gate"
        if pass_count == 0:
            return "confirmed_seed42_failure", "FAIL: confirmed seed-42 0/4 seismic gate"
        return "seed42_transition", f"TRANSITION: seed-42 {pass_count}/4; not a pass"
    if pass_count == n_samples:
        return "all_pairs_pass", f"PASS: all {n_samples}/{n_samples} seismic gates"
    return "incomplete_gate", f"FAIL: only {pass_count}/{n_samples} seismic gates"


def parse_args() -> argparse.Namespace:
    experiment_root = PROJECT_DIR / "experiments/stage4_seismic"
    parser = argparse.ArgumentParser(
        description="Audit one completed Phase-4c seismic strict pair.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--runs-root",
        type=Path,
        default=experiment_root / "runs/cond_generation_0/phase4c_seismic_v1",
    )
...
def _validate_config(
    ...
    *,
    ...
    expected = {
        ...
        "seismic_forward_mode": SEISMIC_FORWARD_MODE,
        "truth_derived": True,
...
def _render_report(summary: Mapping[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# Phase-4c seismic screen",
            "",
            "## Decision",
            "",
            f"**{summary['decision']}**",
            "",
            "This is a truth-derived full-cube synthetic convolutional inverse-crime seismic response, not measured geophysics.",
            "",
            "## Strict pair",
            "",
            f"- Pairing and immutable seismic hashes: `{summary['strict_pairing']}`.",
            ...
            f"- Complete geology-plus-seismic gates: `{summary['pair_gate_pass_count']}/{summary['n_samples']}`.",
            ...
            f"- Hard seismic loss delta: `{float(delta['delta_hard_seismic_loss']):.6f}`.",
            f"- Hard seismic RMSE delta: `{float(delta['delta_hard_seismic_rmse_amplitude']):.6f}` amplitude.",
            ...
            "",
            "A lower seismic residual alone is field fitting, not geological recovery.",
            "",
        ]
    )
...
def main() -> None:
    ...
    paired, reason = paired_seismic_config_verdict(baseline_config, guided_config)
    if not paired:
        raise ValueError(f"strict Phase-4c pairing failed: {reason}")
    ...
    expected_guided_alpha = {"alpha025": 0.25, "alpha010": 0.10}.get(
        args.guided_name
    )
    ...
    _validate_config("baseline", baseline_config, alpha=0.0, **common_validation)
    _validate_config(
        "guided", guided_config, alpha=expected_guided_alpha, **common_validation
    )
    ...
    for sample_id in range(args.n_samples):
        ...
        audit["seismic_loss_improved"] = (
            float(gate_delta["delta_hard_seismic_loss"]) < 0
        )
        audit["passed"] = bool(audit["passed"] and audit["seismic_loss_improved"])
    ...
    if args.n_samples == 1:
        ...
        reranking = {
            ...
            "reason": "implement and run the post-hoc seismic baseline reranking comparator",
        }
        ...
        if reranking.get("status") != "completed":
            classification = "pending_reranking"
            decision = "PENDING: complete post-hoc baseline reranking before promotion"
        elif not bool(
            ...
            decision = (
                "FAIL: guided ensemble mean does not beat reranked baseline seismic residual"
            )

    summary: dict[str, object] = {
        "decision": decision,
        "classification": classification,
        "scope": "truth-derived full-cube synthetic inverse-crime convolutional seismic",
        ...
        "limitations": [
            "synthetic inverse-crime convolutional response is not measured seismic",
            "acoustic codebook gives label 9 deliberately strong observability",
            "normal-incidence convolution omits full-wave physics",
            "continuous seismic loss alone is insufficient",
        ],
    }
    output_dir = args.output_dir or (
        PROJECT_DIR / "experiments/stage4_seismic/reports" / args.run_name
    )
